...
class DoubleDeepQLearner(BaseLearner):
    ...
    def update(self, dataset):
        ...
        # compute target
        next_q_values = self.policy.model(next_states)
        next_q_action = next_q_values.argmax(1)
        next_q_values_target = self.target_policy.model(next_states)
        next_q_value = next_q_values_target.gather(1, torch.LongTensor(next_q_action).view(-1, 1)).view(-1)
